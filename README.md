...
# KK_DBDECoordinateLoadBridge

KK_DBDECoordinateLoadBridge fixes DynamicBoneDistributionEditor data transfer when a Koikatsu coordinate card is selectively loaded through Coordinate Load Option.
...
DBDE and Coordinate Load Option are declared as soft dependencies so this plugin never prevents the game from starting. Both plugins must be installed for the bridge to do useful work. KKAPI is not required by this bridge.

## Installation

Place `KK_DBDECoordinateLoadBridge.dll` in:
...
The KK project targets .NET Framework 3.5 and intentionally uses reflection instead of compile-time DBDE or Coordinate Load Option references.

```powershell
dotnet build KK_DBDECoordinateLoadBridge.csproj -c Release
```
